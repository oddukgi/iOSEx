...
public class PhotoManager: NSObject {
    ...
    class func fetchPhoto(in result: PHFetchResult<PHAsset>, ascending: Bool, allowSelectImage: Bool,
                          allowSelectVideo: Bool, limitCount: Int = .max) -> [PhotoModel] {
    ...
    class func getPhotoAlbumList(ascending: Bool, allowSelectImage: Bool, allowSelectVideo: Bool,
                                 completion: ( ([AlbumListModel]) -> Void )) {
        ...
        arr.forEach { (album) in
            album.enumerateObjects { (collection, _, _) in
                ...
                if collection.assetCollectionSubtype == .smartAlbumUserLibrary {
                    // Album of all photos.
                    let m = AlbumListModel(title: title, result: result, collection: collection, option: option, isCameraRoll: true)
    ...
    public class func getCameraRollAlbum(allowSelectImage: Bool, allowSelectVideo: Bool,
                                         completion: @escaping ( (AlbumListModel) -> Void )) {
        let option = PHFetchOptions()
        if allowSelectImage {
            option.predicate = NSPredicate(format: "mediaType == %ld", PHAssetMediaType.image.rawValue)
        }
        ...
        smartAlbums.enumerateObjects { (collection, _, stop) in
            if collection.assetCollectionSubtype == .smartAlbumUserLibrary {
                let result = PHAsset.fetchAssets(in: collection, options: option)
                let albumModel = AlbumListModel(title: self.getCollectionTitle(collection),
                                                result: result, collection: collection, option: option, isCameraRoll: true)
    ...
    private class func getCollectionTitle(_ collection: PHAssetCollection) -> String {
        if collection.assetCollectionType == .album {
            ...
            if CustomLanguageDeploy.language == .system {
